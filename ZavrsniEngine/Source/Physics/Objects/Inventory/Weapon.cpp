...
namespace objects
{
		...
		void Weapon::shoot()
		{
			...
			{
				_clipCurrent -= 1;
				float rotation = getRotation() + (_spread != 0 ? rand() % _spread - _spread / 2 : 0);
				math::Vector2 destinationPoint = math::Vector2(math::Matrix4::rotation(rotation + getRotation(), math::Vector3(0, 0, 1)) * math::Vector2(_range, 0)); //vektor razlike od origina pucnjave do maksimalnog kraja
